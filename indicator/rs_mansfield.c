...
static int rs_mansfield_feed(struct indicator *i, struct timeline_entry *e) {
  ...
  if((entry = timeline_entry_find(__list_self__(r->ref), e->time))){
    ...
    if(average_is_available(&r->mma)){
      ...
      struct rs_mansfield_indicator_entry *indicator;
      indicator = rs_mansfield_indicator_entry_alloc(value, (mma - last));
      candle_add_indicator_entry(c, __indicator_entry__(indicator));
    }
